...
Система для:
- Учёта ежедневных затрат по ФОТ сотрудников для отелей Волна Sea Village и Art-Life (часы, ставки, начисления)
...
| `records` | Записи информации по сотрудникам (ФИО, часы, начисления) |
...
- fio: ФИО сотрудника
- snils: СНИЛС
- podrazdelenie: Подразделение (отель)
- otdel: Отдел (краткое)
- dolzhnost: Должность
- data: Дата
- chasy: Часы
...
### Проблема: Улучшение навигации в LFL блоке — добавлены кнопки переключения периодов и быстрый выбор месяцев/годов

**Решение:** Добавлены новые элементы управления для удобной навигации по периодам в LFL анализе.

**Новый функционал:**

1. **Кнопки навигации (← →)** для режимов «Месяц к месяцу» и «Неделя к неделе»:
   - Лаконичные стрелки для переключения периодов
   - Клик на «▶» переключает на следующий период
   - Клик на «◀» переключает на предыдущий период
   - Для месяцев: сдвиг на 1 месяц вперёд/назад
   - Для недель: сдвиг на 7 дней вперёд/назад

2. **Быстрый выбор месяцев и годов** под каждым периодом (текущий и предыдущий):
   - Кнопки месяцев: Янв, Фев, Мар... Дек
   - Кнопки годов: 2024, 2025, 2026, 2027
   - Клик по месяцу/году мгновенно меняет период
   - У каждого периода (текущий/предыдущий) свои независимые кнопки выбора

3. **Убрана кнопка «Применить»** из режима «Свой период»:
   - Данные обновляются автоматически при изменении дат
   - Уменьшает количество кликов

**Изменения в коде:**
- `dashboard.html`: добавлен HTML блок `.lfl-nav-buttons` с кнопками навигации
- `dashboard.html`: добавлены блоки `.lfl-quick-select` с месяцами/годами для каждого периода
- `dashboard.html`: добавлены JS функции:
  - `shiftLFLPeriod(direction)` — навигация стрелками
  - `shiftMonthPeriod(direction)` — сдвиг месяца
  - `shiftWeekPeriod(direction)` — сдвиг недели
  - `renderQuickSelectors()` — рендер кнопок месяцев/годов
  - `selectLFLMonth(periodType, monthIndex)` — выбор месяца
  - `selectLFLYear(periodType, year)` — выбор года
  - `updateLFLNavLabel()` — обновление label навигации
- `dashboard.css`: добавлены стили для `.lfl-nav-buttons`, `.lfl-nav-btn`, `.lfl-quick-select`, `.lfl-month-btn`, `.lfl-year-btn`
- `dashboard.html` (`setLFLMode`): скрытие/показ навигации в зависимости от режима
- `dashboard.html` (`updateLFLDisplay`): сохранение периодов в глобальные переменные `lflCurrentPeriod` и `lflPrevPeriod`

**Визуальный результат:**
```
┌─────────────────────────────────────────────────────────────┐
│  📊 LFL Анализ (Like-for-Like)                             │
│  [Месяц к месяцу] [Неделя к неделе] [Свой период]          │
│                                                             │
│       [◀]  Январь 2026  [▶]  ← навигация                   │
│                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────┐ │
│  │ Текущий период  │  │ Предыдущий      │  │ Изменение   │ │
│  │ 150 000 ₽       │  │ 142 000 ₽       │  │ +5.6%       │ │
│  │ 01.01-31.01     │  │ 01.12-31.12     │  │ +8 000 ₽    │ │
│  │                 │  │                 │  │             │ │
│  │ [Янв][Фев][Мар] │  │ [Янв][Фев][Мар] │  │             │ │
│  │ [2025][2026]    │  │ [2025][2026]    │  │             │ │
│  └─────────────────┘  └─────────────────┘  └─────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

**Статус:** ✅ Реализовано (2026-05-24) — навигация по периодам LFL улучшена

### Проблема: При входе на дашборд LFL показывает нули, хотя кнопка «Месяц к месяцу» активна

**Причина:** `calculateLFL()` не вызывался автоматически после загрузки данных. Кнопка «Месяц к месяцу» визуально активна по умолчанию, но расчёт LFL не происходил до первого клика.

**Решение:** Добавить вызов `calculateLFL()` в конец `loadData()` после `applyFilters()`, чтобы при первой загрузке страницы LFL сразу рассчитывал данные для режима по умолчанию.

**Изменения в коде:**
- `dashboard.html` (`loadData`): добавлен вызов `calculateLFL()` после `applyFilters()`

**Было:**
```javascript
applyFilters();
console.log('Calling initEmpGroup()');
setEmpGroup(empGroupMode);
```
...
**Статус:** ✅ Исправлено (2026-05-24) — LFL сразу показывает данные «Месяц к месяцу» при входе
...
**Последнее обновление:** 2026-05-24 (LFL сразу рассчитывается при входе, исправлена пустая анимация LFL, множественный выбор отделов, навигация по периодам LFL)  
**Версия:** 2.5.10  
