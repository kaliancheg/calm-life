...
def _import_records_stream(df, file, sheet_name, progress_callback):
    """Потоковая версия импорта записей с отправкой прогресса"""
    if df.empty:
        return jsonify({'error': 'Файл пустой'}), 400
    ...
    total_rows = len(df_mapped)
    progress_callback(f"data: {json.dumps({'type': 'progress', 'percent': 20, 'text': 'Импорт данных...', 'total': total_rows})}\n\n")
    ...
    for idx, row in df_mapped.iterrows():
        try:
            ...
            if len(batch) >= batch_size:
                ...
                # Отправляем прогресс
                percent = 20 + int(((inserted + skipped) / total_rows) * 70)
                progress_callback(f"data: {json.dumps({'type': 'progress', 'percent': percent, 'text': f'Импорт данных...', 'processed': inserted, 'total': total_rows})}\n\n")
    ...
    progress_callback(f"data: {json.dumps({'type': 'progress', 'percent': 95, 'text': 'Сохранение данных...', 'processed': inserted, 'total': total_rows})}\n\n")
...
def _import_headcount_limits_stream(tmp_path, file, sheet_name, progress_callback):
    """Потоковая версия импорта штатного расписания с отправкой прогресса"""
    try:
        df = pd.read_excel(tmp_path, sheet_name=sheet_name)
        ...
        total_rows = len(df)
        progress_callback(f"data: {json.dumps({'type': 'progress', 'percent': 20, 'text': 'Импорт штатного расписания...', 'total': total_rows})}\n\n")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if inserted % 50 == 0:
                    percent = 20 + int((inserted / total_rows) * 70)
                    progress_callback(f"data: {json.dumps({'type': 'progress', 'percent': percent, 'text': 'Импорт лимитов...', 'processed': inserted, 'total': total_rows})}\n\n")
        ...
        progress_callback(f"data: {json.dumps({'type': 'progress', 'percent': 95, 'text': 'Сохранение...', 'processed': inserted, 'total': total_rows})}\n\n")
        ...
        logger.error(f'Error importing headcount_limits stream: {e}')
        return jsonify({'error': str(e)}), 500
...
def api_headcount_violations():
    ...
    try:
        ...
        # Исключаем отдел "Подработчики" из проверки нарушений
        conditions.append("otdel != 'Подработчики'")
        ...
        # --- Официанты + Мармитчик (Волна) ---
        WAITER_POD = "Волна"
        WAITER_DOLS_RAW = ["Официант", "Официант (улица)", "Мармитчик"]
